...
func (s *Store) GetOrders(ctx context.Context, limit int, offset int) ([]*Order, error) {
	rows, err := s.QueryContext(ctx, "SELECT id, customer_id, status, fulfillment, street_line_1, street_line_2, city, state, zip_code, created_at, updated_at FROM orders LIMIT $1 OFFSET $2", limit, offset)
...
func (s *Store) GetOrder(ctx context.Context, id int) (*Order, error) {
	...
	if err := s.QueryRowContext(ctx, "SELECT id, customer_id, status, fulfillment, street_line_1, street_line_2, city, state, zip_code, created_at, updated_at FROM orders WHERE id=$1", id).Scan(&order.ID, &order.CustomerID, &order.Status, &order.Fulfillment, &order.StreetLine1, &order.StreetLine2, &order.City, &order.State, &order.ZipCode, &order.CreatedAt, &order.UpdatedAt); err != nil {
		return nil, err
	}
...
func (s *Store) CreateOrder(ctx context.Context, order *Order) (int, error) {
	var id int
	if err := s.QueryRowContext(ctx, "INSERT INTO orders (customer_id, status, fulfillment, street_line_1, street_line_2, city, state, zip_code) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id", order.CustomerID, order.Status, order.Fulfillment, order.StreetLine1, order.StreetLine2, order.City, order.State, order.ZipCode).Scan(&id); err != nil {
		return 0, err
	}
	return id, nil
}
...
func (s *Store) CreateOrderItems(ctx context.Context, orderItems []*OrderItem) error {
	...
	for _, item := range orderItems {
		_, err := tx.ExecContext(ctx, "INSERT INTO order_items (order_id, product_id, price, quantity) VALUES ($1, $2, $3, $4)", item.OrderID, item.ProductID, item.Price, item.Quantity)
...
func (s *Store) UpdateOrderStatus(ctx context.Context, id int, status string) error {
	_, err := s.ExecContext(ctx, "UPDATE orders SET status=$1 WHERE id=$2", status, id)
